...
def speak_to_microphone(api_key, region):
   ...
   # Start time for E2E latency
    start_time = time.perf_counter()  # <-- Added line

    first_token_received = False  # <-- Added line
    ttft = None  # <-- Added line
    ...
    while True:
        ...
        if not first_token_received:  # <-- Added block
            ttft = (time.perf_counter() - start_time) * 1000  # Time to First Token in milliseconds
    ...
    # End time for E2E latency
    end_time = time.perf_counter()  # <-- Added line

    # End-to-End Latency in milliseconds
    e2e_latency = (end_time - start_time) * 1000  # <-- Added line

    print(f"Time to First Token (TTFT): {ttft:.2f} ms")  # <-- Added line
    print(f"End-to-End Latency (E2E Latency): {e2e_latency:.2f} ms")  # <-- Added line
